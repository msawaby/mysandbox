...
Ampl=0.38 /2;          	% Input signal amplitude [V]
const=0;
...
Cf=2.5;				% Integrating Capacitance of the first integrator
...
noise1=30e-9*0;			% 1st int. output noise std. dev. [V/sqrt(Hz)]
b=1;
%echo off;
...
sim('Sigma_Delta_NLDAC_SL', (N+Ntransient)/Fs, options);	% Starts Simulink simulation
...
DAC_Distortion = yy_dac ./ DAC_step - yy12 - 1 + (N/sum(w))*sinusx(yy12(1:N).*w',f,N);
out_bias = sum(yy12)/length(yy12);% - sum(yy_dac)/length(yy_dac);
yy1 = yy12-out_bias;
%echo off;
...
[snr_dac,ptot_dac,psig_dac,pnoise_dac]=calcSNR(DAC_Distortion,f,3,fB,w',N);
